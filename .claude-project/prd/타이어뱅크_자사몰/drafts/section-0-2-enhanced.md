...
| **ReservationStatus** | `PENDING(0)`, `CONFIRMED(1)`, `IN_PROGRESS(2)`, `COMPLETED(3)`, `CANCELLED(4)` | 장착 예약 상태. PENDING: 예약 접수 대기, CONFIRMED: 장착점 확인 완료, IN_PROGRESS: 장착 진행 중(장착 시작 후 완료 전), COMPLETED: 장착 완료 처리, CANCELLED: 예약 취소 |
| **PartnerStatus** | `PENDING(0)`, `ACTIVE(1)`, `REJECTED(2)`, `SUSPENDED(3)`, `WITHDRAWN(4)` | 장착점 파트너 상태. PENDING: 승인 대기, ACTIVE: 정상 운영, REJECTED: 승인 거절(관리자가 신청을 거절한 상태), SUSPENDED: 운영 정지, WITHDRAWN: 탈퇴 |
