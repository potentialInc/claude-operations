...
| AWS S3 (또는 호환 스토리지) | 스크린샷 버전 이미지 저장, 코멘트 첨부파일 저장, Presigned URL로 직접 업로드/다운로드 |
...
|--------|-----------|-------------|
| **Organization** | `id` (PK, UUID), `name` (VARCHAR 100, NOT NULL), `slug` (VARCHAR 50, UNIQUE, NOT NULL), `status` (SMALLINT, NOT NULL, default 1), `plan_id` (FK → Plan), `owner_id` (FK → User), `created_at`, `updated_at`, `deleted_at` | 멀티테넌시의 최상위 단위. `slug`는 URL 식별자. `status`는 OrganizationStatus enum |
