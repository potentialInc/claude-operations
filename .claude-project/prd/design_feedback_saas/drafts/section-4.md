...
- 좌측 사이드바: 어드민 전용 네비게이션 (대시보드 / 조직 / 사용자 / 프로젝트 / 감사 로그 / 설정)
...
| 조직 상태 분포 | Donut Chart | — | active / inactive / suspended |
...
| 필터 | Status Dropdown | All / Active / Inactive / Suspended |
...
| Bulk Action | Dropdown | 선택된 행에 일괄 적용 (비활성화, 정지, 삭제) |
...
| 상태 | Enum Badge | ✓ | Active(녹) / Inactive(회) / Suspended(빨) |
...
| 액션 | 대상 | 확인 Dialog |
|------|------|------------|
| 비활성화 | 선택된 조직 전체 | "N개 조직을 비활성화하시겠습니까? 해당 조직 사용자는 로그인이 제한됩니다." |
...
| 상태 | Enum Badge | Dropdown 편집 가능 (Active / Inactive / Suspended) |
| 설명 | Text | 인라인 편집 가능 |
...
- 상태 변경: Active ↔ Inactive ↔ Suspended 전환, 확인 Dialog 표시
...
**프로젝트 탭:**
- 각 프로젝트 행 클릭 시 `/admin/projects` 상세로 이동
...
**가능한 액션:**
- 상태 변경 (Active / Inactive / Suspended): 확인 Dialog 후 실행
...
|--------|------|------|------|
| Checkbox | Boolean | - | |
| 프로젝트명 | String | ✓ | 프로젝트 상세 링크 (읽기 전용 조회) |
