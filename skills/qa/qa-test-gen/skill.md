...
| ORM | Column Definition Pattern | Example |
|-----|--------------------------|---------|
| TypeORM | `@Column({ type, nullable, length })` | `@Column({ type: 'varchar', length: 100, nullable: false })` |
| Prisma | `field Type @db.Modifier` in `schema.prisma` | `name String @db.VarChar(100)` |
| Sequelize | `DataTypes.TYPE(length)` + `allowNull` | `name: { type: DataTypes.STRING(100), allowNull: false }` |
| Mongoose | `{ type: Type, required, maxlength }` | `{ type: String, required: true, maxlength: 100 }` |
| Django ORM | `models.Field(max_length, null)` | `models.CharField(max_length=100, null=False)` |
| SQLAlchemy | `Column(Type(length), nullable)` | `Column(String(100), nullable=False)` |
| Drizzle | `type('name', { length })` | `varchar('name', { length: 100 })` |
| Eloquent | Migration: `$table->type('name', length)` | `$table->string('name', 100)->nullable(false)` |
