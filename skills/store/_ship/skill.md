...
Phase 1 (prep) — Information Gathering:
  ✅ Completion Conditions:
    - store-prep/app-info.md exists
    - store-prep/listing-google-play.md exists
    - store-prep/listing-app-store.md exists
    - store-prep/privacy-policy.md exists
    - store-prep/client-guide.md exists
    - store-prep/tech-checklist.md exists

Phase 2 (assets) — Asset Preparation:
  ✅ Completion Conditions:
    - store-prep/assets/icons/ directory contains icon-playstore.png (512x512) exists
    - store-prep/assets/icons/icon-appstore.png (1024x1024) exists
    - store-prep/assets/screenshots/  contains at least 2 images
  🔄 Partially Complete:
...
Phase 4 (deploy) — Production Deployment:
  ✅ Completion Conditions:
    - store-prep/deploy/deploy-checklist.md exists
    - store-prep/deploy/env.production.example exists
  Additional Detection:
...
Phase 5 (build) — Release Build:
  ✅ Completion Conditions:
    ...
    - store-prep/build/ directory contains build guide
  Additional Detection:
...
Phase 6 (submit) — Store Submission:
  ✅ Completion Conditions:
    - store-prep/submit/ directory contains submission guide
    - store-prep/submit/pre-submit-checklist.md exists
  🔶 Blocker:
...
1. **Project Scan**:
   ...
   Glob: store-prep/assets/icons/*.png → Icon preparation status
   Glob: store-prep/assets/screenshots/*.png → Screenshot status
   ...
   Read: store-prep/pipeline-status.json → Previous state
   ```
...
3. **Output**: Saved to `store-prep/checklist.md`
...
### `store-prep/pipeline-status.json`
...
4. Save to `store-prep/checklist.md`
...
- Save release notes to `store-prep/build/version-history.md`
